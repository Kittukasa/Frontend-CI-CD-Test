...
const loadContacts = async () => {
  ...
  const handleImportFile = async (file: File | null) => {
    ...
    if (nonEmptyHeaderKeys.length < 1 || nonEmptyHeaderKeys.length > 2) {
      setImportError(
        `Only one or two columns are allowed for now: ${requiredHeaderLabel}${optionalHeaderLabel ? `, ${optionalHeaderLabel}` : ''}. Remove extra columns and try again.`
      );
      return;
    }

    const allowedKeys = new Set(['mobilenumber', 'mobile', 'phonenumber', 'phone', 'customername', 'name']);
  ...
  const handleDeleteContact = async (contact: ContactRecord) => {
    ...
    if (!phone) {
      toast({ title: 'Delete failed', description: 'Missing phone number.', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      setContacts(prev => prev.filter(item => (item.normalized_phone || item.phone || '') !== phone));
      await loadContacts();
    } catch (err) {
...
`;
  ...
  const submitImport = async () => {
    ...
    if (nonEmptyHeaderKeys.length < 1 || nonEmptyHeaderKeys.length > 2) {
      ...
      const unexpectedHeaders = parsed.headerKeys.filter(key => key && !allowedKeys.has(key));
      if (unexpectedHeaders.length > 0) {
        setImportError(`Only these columns are allowed for now: ${allowedHeadersLabel}. Remove extra columns and try again.`);
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 text-gray-900">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Customers from your customer records (named customers only). Use this list for campaigns or outreach.
            </p>
          </div>
                ...
                <tbody className="divide-y divide-gray-100 bg-white text-gray-700">
                  {sortedContacts.map(contact => (
                    <tr key={contact.contact_id}>
                      <td className="px-6 py-4 font-medium">
                        <div className="flex items-center gap-2">
                          ...
                          {contact.source === 'import' && (
                            <span className="inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-semibold text-indigo-700 border border-indigo-100">Imported</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-gray-500">{formatDateTime(contact.created_at)}</td>
                      <td className="px-6 py-4 text-gray-500">{formatDateTime(contact.updated_at)}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Button
                          type="button"
                        ...
                        <Button
                          ...
                          disabled={deletingPhone === (contact.normalized_phone || contact.phone || '')}
                        >
      ...
      <Dialog open={Boolean(editContact)} onOpenChange={(open) => { if (!open) setEditContact(null); }}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 text-sm">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wide text-gray-600">Phone</label>
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wide text-gray-600">Tag</label>
              <input
                type="text"
                value={editTag}
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setEditContact(null)} disabled={savingEdit}>
              Cancel
            </Button>
            <Button type="button" onClick={handleEditContactSave} disabled={savingEdit || !editName.trim()}>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className="max-w-xl">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button type="button" variant="outline" onClick={downloadSampleCsv} disabled={importing}>
                  Download sample
                </Button>
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={submitImport} disabled={importing || importValidCount === 0}>
