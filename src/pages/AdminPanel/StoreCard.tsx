    ...
    <Link to={`/admin-panel/stores/${store.storeId}`} className="group block focus:outline-none">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 sm:gap-6">
        ...
        <div className="flex flex-col items-end gap-2">
          <span className={`rounded-full px-4 py-1 text-xs font-semibold uppercase tracking-wide ${palette.badge}`}>
            {store.health === "healthy" ? "Healthy" : store.health === "watch" ? "Watch" : "At risk"}
          </span>
          {store.badgeLabel && (
